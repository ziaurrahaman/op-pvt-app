...
import 'package:OpPvt/utils/database_client.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Future<void> initializeFavourite() async {
  //   final DatabaseClient _databaseClient = DatabaseClient.instance;
  //   favuser = await _databaseClient.getFavouriteUser(userId, currentUser);
  // }
  ...
  Future<void> createUsersAcordingToFav(
    ...
    if (isFirstTime == false) {
      for (int i = 0; i < snapshot.data.documents.length; i++) {
        if (_databaseClient.getFavouriteUser(userId, currentUser) == null) {
          users.add(User(
        ...
        //   if (userFromSnapshot.userId != favUser.userId) {
        //     users.add(User(
        //   bio: snapshot.data.documents[index]['bio'],
        //   dob: snapshot.data.documents[index]['dob'],
        //   email: snapshot.data.documents[index]['email'],
                      ...
                      builder: (context, snapshot) {
                        ...
                        return PageView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            // index1 = index;
                            // widget.pageCallback(pageController);

                            // final user = User.fromMap();
                            // print(
                            //   snapshot.data.documents[index]['subscribers'],
                            // );
                            // createUsersAcordingToFav(snapshot, index);
